package edu.research.neiu.collection;
...
public class DataCollector {
	...
	private boolean verbose, geoloc;
...
//		displayMapData(destServers, "destination");
		...
		if (checkStart.equalsIgnoreCase("exit")) {
...
//								System.out.println();
	...
	private void createOutputFile() {
		File checkUniqueNumber = new File("Collection/config/checkUniqueNumber");
		Integer uniqueNum = -1;
		try
		...
		cal = Calendar.getInstance();
		outputFile = new File("Collection/Output/datasample_" + Integer.toString(uniqueNum) 
	...
	private void loadFiles() {
		File sourceFile = new File("Collection/config/source.txt");
		sourceServers = loadData(sourceFile, "source");

		File destFile = new File("Collection/config/destination.txt");
...
//				System.out.println(trimMsg);
						...
						if (words[0].equals("")) {
							...
							if ((!words[3].equals("") && !words[1].equals("")) || (!words[4].equals("") && !words[1].equals(""))){
								...
								if (words[3].equals("*") && !words[4].equals("*")) {
									...
									if (!words[4].equals("") || !words[4].equals(" ")) {
										...
										if (geoloc) {
											String asn = getAsn(words[4]);
											String loc = getLoc(words[4]);
											
											if (verbose) {
												System.out.println("ASN="+asn);
												System.out.println("Loc="+loc);
											}
											this.write("\t\t\t\t<loc>" + loc + "</loc>\n");
											this.write("\t\t\t\t<asn>" + asn + "</asn>\n");
										}
									}
								}
								this.write("\t\t\t</data>\n");
							...
							if (words[2].equals("*") && !words[3].equals("*")) {
								...
								if (!words[3].equals("") || !words[3].equals(" ")) {
									...
									if (geoloc) {
										String asn = getAsn(words[3]);
										String loc = getLoc(words[3]);
	...
	private String getLoc(String ipv4Addr) {
		String loc = "";
		
		if (ipv4Addr.equals("*") || ipv4Addr.equals("") || ipv4Addr.equals(" "))
			return loc;
		
		HttpClient client = new DefaultHttpClient();
		HttpGet get = new HttpGet("http://ipinfo.io/" +(ipv4Addr + "/geo"));
		...
		try {
			...
			if (locations[0].contains("Rate limit"))
				...
				if (!locations[4].contains("null") && locations[4] != null && (locations[4].length() > 14)) {
					loc = locations[4].substring(14, (locations[4].length() - 2));
					if (!locations[3].contains("null")) {
						loc = loc + " - " + locations[3].substring(13, (locations[3].length() - 2));
						if (!locations[2].contains("null"))
							loc = loc + " - " + locations[2].substring(11, (locations[2].length() - 2));
					}
				}
			}
			return loc;
	...
	@SuppressWarnings("resource")
	private String getAsn(String ipv4Addr) {
		String asn = "";
		
		if ((ipv4Addr.equals("*") || ipv4Addr.equals("") || ipv4Addr.equals(" ")) || !geoloc)
			return asn;
		
		HttpClient client = new DefaultHttpClient();
		HttpGet get = new HttpGet("http://ipinfo.io/" +(ipv4Addr + "/org"));
